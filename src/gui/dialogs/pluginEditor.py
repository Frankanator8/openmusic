...
# Plugin editing dialog
class PluginEditor(QDialog):
    def __init__(self, parent):
        super().__init__(parent)
        self.setWindowTitle("Manage Plugins/Styles")
        self.setModal(True)
        # similar style to Playlists in that you have to enable/disable Plugins and can reorder them
        self.uidWidget = {}
        ...
        # More info about plugins and how to enable/reorder them
        ...
        # Make sure when order is changed it saves
    ...
    # Open right click menu to delete a plugin or open its home folder
    ...
    # save the current enabled/order state
    def save(self):
    ...
    # next 2 are both system file requests
    ...
    # clear a QLayout to make way for new stuff
    def clear_layout(self, layout):
        while layout.count():
            ...
            if child.widget():
                ...
                self.clear_layout(child.myLayout())
                child.myLayout().deleteLater()

    # reload the left-side plugin list to reload all available plugins
